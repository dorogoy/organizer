...
**Postscript — 2026-08-28.** In this addendum's own supersession style, item (4) above is itself superseded: the motion-dashes lever was dissolved outright — no threshold, no register; the seed draws at rest at every size and on every surface (`DESIGN.md`, dissolution record under `{components.seed-glyph}`; the Story 1-2 amendment spec `_bmad-output/implementation-artifacts/spec-1-2-remove-seed-motion-dashes.md` is the renegotiation record). Warm Return's 168px dashes-on rendering named in item (4) is history. The measured facts this report carries — 17 filaments, 7 detached dashes in the reference — stand untouched.
...
- **motion dashes: barred from the destination trio** (`DESIGN.md` said "No motion dashes in this row" — a sentence dissolved into the general at-rest rule on 2026-08-28; the bar it named is subsumed by "never, at any size");
