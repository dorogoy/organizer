...
| Energy check-in | Dispenser — the ambient strip, first opening of the day | Three battery marks — llena / media / baja — three direct taps (FR-4). Exists to ask for **less**: media filters nothing, only baja narrows. Once a day: answered or dismissed, it is gone until tomorrow. While the weekly self-report is pending it holds the strip at any day's first opening, not only Sunday's; once it resolves, the check-in may take the slot in the same opening. |
...
| **Anti-Marathon checkpoint** | Dispenser, at every multiple of the checkpoint interval inside a session | `Nada más por el momento` is the primary permission to stop; `Quiero seguir` is available, silent and secondary. Sessions shorter than one interval reach no checkpoint: their close is the permission to stop, and when the final checkpoint coincides with the close, that close is the offer (FR-10, UJ-1). |
| **Before/After reward** | Dispenser, on completing a transformation | With a Before photo: a private, local side-by-side pair labelled `Antes` and `Después`, with no caption or share action. Without one: `Un trabajo estupendo`. |
...
| **Decluttering Protocol** | Dispenser, when the dealt Micro-task is a decision about an object | The two detachment questions, verbatim per FR-20, each require `Sí` or `No`; then one destination is required. |
...
| `¡Buen trabajo!` | Completion acknowledgement — explicit narrow exclamation exception |
...
| `Si no tienes nada que hacer, tengo un trabajillo para ti` · `Hay algo en la casa que se puede mejorar` · `Con solo 15 minutos podemos avanzar` · `Si necesitas cambiar de tarea, te puedo ayudar` · `Te puedes tomar un respiro de 15 minutos haciendo otras cosas` | Ambient Invitation rotating notification set |
| `Nada más por el momento` · `Quiero seguir` | Anti-Marathon primary message and silent secondary action |
| `Siempre a tu disposición` | Warm Return |
...
| `La foto se procesará por [proveedor] para obtener las tareas necesarias.` · `Enviar la foto` · `No enviarla` | Per-scan consent gate — one parameterized localized body and equal actions |
| `Creando tareas` | Scan wait, with an indeterminate animated pencil writing |
...
| `Analizar` · `Volver` | Typed project genesis — consent action and quiet exit |
| `Ajustar grupos de tareas` | One-time first-run curation invitation |
| `Un trabajo estupendo` | Transformation reward when no Before photo exists |
| `Antes` · `Después` | The only labels on a Before/After pair |
| `Está bien así.` | Snowball suggestion dismissal |
...
| Alarm-free and streak-free; reserve an exclamation solely for `¡Buen trabajo!` after `Hecho`. | Red, warning iconography, exclamation marks outside that narrow recognition, badges, overdue language. |
| In a Before/After pair, show only `Antes` and `Después` outside the frames. | Any caption or adjective about the result — *mejor*, *más despejado*, *casi*. An adjective is a scale, and a scale brings back the deficit. |
...
| `{components.duration-chip}` | Dispenser eyebrow; pocket trigger | Reading order is the mechanism — the cost is read before the ask. Never a countdown, never a clock. A pocket declared while a card is in progress behaves like baja: the card in progress can be finished, and the re-filter applies to the next deal. |
...
| `{components.destination-flow}` | 3-Destination Flow | Three equal-weight choices at `{spacing.glyph-destination}`, `{spacing.destination-row-gap}` between rows, one decision per screen, nothing else on it. No default, no pre-selection, no ordering signal. The user must choose one destination after answering both detachment questions. |
| `{components.zone-marker}` | Dispenser footer | Names the one active FlyLady zone for today, in `{typography.support}`. A place-marker, not a control — it is not a filter and opens nothing. With no active zone — every zone cluster disabled under curation — the footer is simply absent, never an empty placeholder. |
...
| Microphone affordance | Manual Capture, and nowhere else | An **explicit press** starts dictation and a **second press ends it** — a tap toggle, so `Escuchando…` always has an exit. Nothing listens outside the press; the rest of the mechanics — permission timing, on-device recognition with no cloud fallback, no retained audio, one utterance one Micro-task, nothing parsed from the transcript, origin `manual`, the Settings-only boolean — are FR-32's, inherited by reference. The transcript lands **in the existing one-line field on the existing surface** — never a confirmation screen — and FR-27's correct-or-discard affordance governs a mis-hearing, which makes that affordance load-bearing. An empty recognition leaves the line as it was: the confirming action stays disabled until text, as everywhere. The mark is the capsule glyph at the field's end (`{components.microphone-glyph}`); while dictating it declares itself in ink and prose — blue mass plus the caption `Escuchando…` — and **never by motion** (the breathing cue was offered and declined). **The keyboard is never removed**; correcting a transcript requires it. Where on-device Spanish recognition is unavailable, the affordance is **simply absent** — the language-pack pointer stays absent-and-silent, by decision 2026-08-27 — and refusing the permission leaves keyboard capture fully functional, with the app never asking again on its own. |
...
| `{components.icon-glyph}` · `{components.seed-glyph}` · `{components.destination-mark-dark}` | Throughout | Drawing specs with no behaviour of their own. `destination-mark-dark` is the dark-mode form of the destination trio; light/dark follows the system theme (Settings, Information Architecture). |
...
- **Proportion does the work a score would do.** Two `{components.photo-frame}` plates of **equal size, at equal height, with the same corner**, `{spacing.photo-pair-gap}` apart, and the labels `Antes` / `Después` **outside** the pastel rather than on it. That layout makes the comparison legible without anyone grading it. The moment one plate is larger, or higher, or framed differently, the layout starts having an opinion — and an opinion is a rating. This is what replaces the percentage, the star, the score and the progress bar, none of which exist.
- **No caption and no sharing.** The pair carries only `Antes` and `Después`; it remains private and local. If no Before photo exists, no one-plate diff or placeholder appears: the completion reward is simply `Un trabajo estupendo`.
...
| **Warm Return** | Dispenser | A rebalanced plan. `Siempre a tu disposición` appears without backlog or any reference to missed days. The days away are not representable in the schema, so they cannot appear. |
...
| Anti-Marathon checkpoint | Checkpoint | Rest offered at **every multiple of the checkpoint interval** within the session; a session shorter than one interval reaches none, and its close is the permission to stop (FR-10). When the final checkpoint coincides with the close, that close is the offer (UJ-1). `Nada más por el momento` is the primary permission; `Quiero seguir` is available, silent and secondary, never highlighted or animated. |
...
| Scan wait | Scan | In-app, with `Creando tareas` and an indeterminate animated pencil writing. There is **no latency cap** and deliberately **no timeout**. Leaving the surface or backgrounding the app **cancels the wait and discards it**; a failure is stated explicitly through the existing no-Slicer/degradation path. |
...
| Empty Album / empty dashboard | Album, dashboard | Neither surface exists until the first transformation; no empty state or copy is shown. |
| Pool exhausted (mid-session or at a first opening) | Dispenser | The session closes early — or a first opening that finds nothing left to deal simply shows the close — with the fixed warm string `por hoy no hay nada más que merezca la pena`: never an error, never an empty state styled as absence or debt (FR-3). Part of the SM-C2 audit table. |
...
- **Banned everywhere except the explicit completion acknowledgement:** lists, calendars, backlogs, counts of anything undone, streaks, badges, overdue anything, red as alarm, warning iconography, exclamation marks other than `¡Buen trabajo!` after `Hecho`, `¿seguimos?`, "keep going" as a primary action, a blanket "always allow" consent setting, always-on listening or ambient audio of any kind, and any motion that carries a completed card toward a counter, a pile or a badge.
...
- **Genesis text has its own consent wording.** The manual-entry screen states, in plain Spanish, that the typed or form description will be analysed to create tasks. `Analizar` is the consent action; unlike photo consent, this surface does not name the provider and has no separate dialog.
...
*Note:* the Warm Return copy is `Siempre a tu disposición`, per the fixed-string table — and it never names the absence it is responding to.
...
2. He presses the microphone affordance, says it out loud — `regar las plantas de la vecina` — and presses again when he is done; the words land in the line, **in the existing one-line field on the existing surface**. No confirmation screen exists to receive them. Had he preferred typing, the keyboard is right there and always is (FR-32).
...
1. **Whether the seasonal suggestion engine needs any configuration surface in v1** (PRD OQ-6), or pure defaults. Defaults-only is the standing answer until a suggestion actually misfires — which is a position, not a decision.
2. **Screen-reader behaviour.** TalkBack labels, roles, state announcements and traversal order are never discussed anywhere in the record.
3. **The departure trajectory of the completed card.** That it exits entirely and never flies toward a counter, pile or badge is settled; whether it departs along the global 45° offset axis, and whether the departure itself is the celebration, is proposed and unconfirmed.
4. **The second locale.** FR-9 says no interrupted, incomplete or overdue state is displayed "in either language". The second language is never named, and its string lengths bear on the 200% fold. The flat i18n-ready string table (§7) already makes a second locale translation rather than redesign.
5. **The "plates but not the meal" copy (PRD OQ-12).** Does any copy explain why the app knows about clearing the table and not the cooking that produced it, and where would it live? It is a copy decision on a surface this spine sites, and it has no answer.
