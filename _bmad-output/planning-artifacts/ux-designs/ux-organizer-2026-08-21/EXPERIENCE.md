...
| **Project genesis** | Dispenser — the single quiet affordance, as its **recommended action** | Two paths. The photo path is the recommended action: photograph a real space and the Slicer cuts it into steps. **Manual entry sits beside it and is always available** — a typed description seeds the same Epic Project. Photo is an invitation, never a gate. The manual-entry screen states in plain Spanish what is sent and to which provider — **the send action is the consent** (FR-25, §7: the egress map's second payload). |
...
Ambient, dismissible in one tap, living on the Dispenser rather than on surfaces of their own: the seasonal activation suggestion, the six-month Quarantine follow-up, and the snowball suggestion. The Sunday self-report `Esta semana, ¿cuánto te ha agobiado la casa?` is ambient too, but **persists until answered that week** (SM-2): it appears on the first opening from Sunday onward, a dismissal hides it for that opening rather than for the week, and it is **never notified** — a notification would violate FR-24 and §5.2. A week with no answer simply has no data point.
...
**Both amendments are paid into the PRD** — 2026-08-26: §1.1 now carries a named exception register, **E1** (the template list) and **E2** (the multi-path genesis zone), each citing this session, with FR-11 and FR-31 carrying the consequences. The upstream edit this paragraph used to demand is closed; what remains here is the record of the arithmetic behind it.
...
| Energy control | Dispenser | One tap, three values. Re-filters the pool in under 500 ms. 🟢 and 🟡 filter nothing — only 🔴 narrows, to Instant Habits and ≤ 60 s; a day of middling energy is not a smaller day (FR-4). Tapping 🔴 mid-task behaves like the checkpoint: the card in progress can be finished, and the filter applies to the next deal — work in progress is never withdrawn without comment. Only ever narrows what is asked of the user; never unlocks anything. |
...
| Ambient suggestion | Dispenser | Seasonal activation, snowball, Quarantine follow-up. Dismissible in one tap; never a primary action; at most one visible at a time. The snowball suggestion is suppressed when the Time Bag sits at the top of its range — there is nothing left to suggest (FR-23). |
...
| Genesis paths | Project genesis | Manual entry is **never gated** behind the photo: reaching it costs one tap and asks for no reason. The Archetype Template list opens from the manual-entry screen only, and selecting a template is a tap like any other — no confirmation step; what it does is enable or disable Evergreen clusters (FR-31), nothing more. The manual-entry screen states what is sent and to which provider: **the send action is the consent** (FR-25, §7). |
...
5. On the same surface, ambient: `Esta semana, ¿cuánto te ha agobiado la casa?` — it persists until answered that week and is never notified (SM-2). He answers it, or he doesn't; a week with no answer simply has no data point.
...
6. **Cluster curation presented without becoming NL-1's list** (PRD OQ-3's surviving half, routed here): onboarding and Settings carry the same capability as the E1 template surface, and none of the three may read as a browsable catalogue (FR-31). The wording of zones and cluster names, the row shape, the count and whether it scrolls — all unspecified.
...
9. **Onboarding / first-run shape.** The PRD now fixes one property: cluster curation is reachable from onboarding, is inherently optional, and abandonment mid-curation leaves every cluster active — the first composed day is never empty (FR-31). Everything else about the surface — order, tone, length, whether it holds anything beyond curation — is unanswered.
...
11. **Settings IA.** Settings is now *sited* — it is the way out of the Dispenser's single quiet affordance — but its **contents** are only enumerated by the PRD and were never organised: Time Bag, AI Access Path (allowlist with verified-terms dates, FR-28), Ambient Invitation, export destination **and import/restore (FR-30)**, **cluster curation (FR-31)**, **the dictation boolean (FR-32)**, album purge, export failure.
...
20. **Roughly twelve strings drafted in [`mockups/key-screens-1.html`](mockups/key-screens-1.html) and never authorised.** They are drafts on a mockup page, not spine content, and the structural rule in Voice and Tone applies to them before anything is wired: every string externalised, nothing concatenated at runtime, the whole set auditable as one flat table. Listed so none is adopted by accident: `Guardar` · `Descartar` (Manual Capture, plus that screen's title, helper text and example) · `Enviar la foto` · `No enviarla` (consent gate, plus the gate's explanatory body) · `Empezar con esta` · `Volver` (genesis) · `Guardar en el álbum` · `Cerrar` (Before/After) · `Quitar esta sugerencia` (snowball) · and the Before/After caption itself, which is bounded by the ceiling above but not written. These are additional to the seven no-Slicer strings and the copy items above, not a restatement of them. (`Ver otra` is removed from this list — it belonged to the rejected one-at-a-time deck and no string of that shape exists.)
...
22. **The capture-copy questions the PRD routes here (OQ-11's open half, OQ-12).** May the capture surface's copy say that the line holds no dates — given that saying it also tells the user their deadline does not belong here (the line is plain text that models nothing, FR-27)? And does any copy explain why the app knows about the plates and not the meal, and where would it live? Both are copy decisions on surfaces this spine sites; neither has an answer, and they should be answered together.
23. **The Settings pointer to on-device recognition (PRD OQ-13).** Where recognition is unavailable the microphone is absent and silent. May Settings carry a one-time pointer to the system's own language-pack installation — given that a pointer is a small piece of pending work planted in the one surface §7 reserves for the validator? Absent-and-silent is the standing answer; the argument against it is real (someone who never sees a microphone concludes the app has no voice at all).
24. **The Micrófono glyph (see `DESIGN.md` Open Questions).** FR-32's dictation affordance needs a mark; the nine-glyph set has none, and the two-plate treatment constrains the drawing. Blocks the Manual Capture surface's final layout, not its behaviour — the behavioural row is already in Component Patterns.
