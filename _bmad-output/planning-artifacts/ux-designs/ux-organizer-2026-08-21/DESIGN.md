...
colors:
  ...
  icon-mass-blue: '#9BC1D2'
  # ---- DARK MODE · hand-authored, never an inversion · icon masses at L* 62 ----
  ...
  icon-mass-neutral-dark: '#8E9894'
  icon-mass-ochre-dark: '#A2947E'
  icon-mass-blue-dark: '#7B9BA9'
  dest-keep-dark: '#7E9C91'
  dest-donate-dark: '#9793AA'
  dest-trash-dark: '#AE8E91'
typography:
  task:
    fontFamily: 'Lora'
    fontSize: '26sp'
...
components:
  ...
  size-option:
    use: 'the three-size picker in Manual Capture — the only selector attached to a capture'
    typography: '{typography.duration}'
    radius: '{rounded.full}'
  ...
  destination-flow:
    ...
    typography: '{typography.destination-label}'
  ...
  destination-mark-dark:
    ...
    form: 'the light form unchanged — two plates, the global offset, mass under line; line in {colors.ink-primary-dark}, mass in the dark hue'
    keep-mass: '{colors.dest-keep-dark}'
    donate-mass: '{colors.dest-donate-dark}'
    trash-mass: '{colors.dest-trash-dark}'
  icon-glyph:
  ...
  energy-checkin:
    use: 'the daily energy check-in strip — the only place the battery marks live'
    marks: 'three batteries: llena / media / baja — casing + nub line-only, charge a registered mass filling from the left'
    charge-widths: 'llena 12.8u · media 6.4u · baja 3.2u (24 viewBox)'
    selected: 'charge {colors.icon-mass-blue} + casing {colors.ink-primary}'
    unselected: 'charge {colors.icon-mass-neutral} + casing {colors.ink-secondary}'
    offset: 'none — registered fill, a documented exception (closed container reads as leak; horizontal axis against the 45° vector is transverse)'
    tap-target: '{spacing.touch-target-min}'
    default: 'llena pre-marked — the standing default'
  microphone-glyph:
    use: "FR-32 dictation affordance — Manual Capture, at the one-line field's end, and nowhere else"
    form: 'vertical capsule + stem + U-arc stand, line-only in ink-primary'
    mass: 'the capsule, registered — its vertical axis contradicts the global 45° vector; {colors.icon-mass-neutral} at rest'
    active: '{colors.icon-mass-blue} while dictating, declared also by the caption Escuchando… in {typography.support}'
    motion: 'none — the breathing cue was offered and declined; state is ink + prose, never animation'
    size: '24px inside {spacing.touch-target-min}'
    absent: 'simply absent where on-device recognition is unavailable — never greyed, never explained'
  ambient-strip:
    use: 'the single strip below {components.dispenser-card} — six residents, one spec'
    typography: '{typography.support}'
    ink: '{colors.ink-secondary}'
    dismiss: '✕ at {spacing.touch-target-min}'
    chrome: 'ephemeral residents bare; persistent residents 1px {colors.border-hairline}'
    density: 'at most one visible at a time'
  curation-row:
    use: 'cluster curation — the E1 template surface, onboarding one-time offer, and Settings; one component, three homes'
    row: 'cluster name + cadence in {typography.support} + platform switch, tappable anywhere'
    cadences: 'diaria / semanal / mensual-estacional'
    feedback: 'none — enabling or disabling produces no count, summary or copy (FR-31)'
    level: 'cluster only, never a task row'
  photo-frame:
  ...
  dashboard-highlight-row:
    ...
    known-degradation: 'if ever left unreflowed at 200% the row stops reading as a row — measured arithmetic in Typography, the 200% floor'
---
...
| Icon mass | `dest-keep` `dest-donate` `dest-trash` `icon-mass-neutral` `icon-mass-ochre` `icon-mass-blue` | L\* 76.0, Δ L\* 21.6 against `{colors.surface-base}` | The colour plate inside a glyph, nowhere else |
...
- **`icon-mass-ochre` #CAB9A0** — the Hoja's mass, the first hue admitted outside the destination trio (the blue below is the second). **L\* 75.94, chroma 15.02, hue 82.3**, computed by the trio's own method (CIE Lab, D65, sRGB; separation measured as distance in the a–b plane, which at equal L\* is exactly ΔE) rather than picked by eye. It lands in the system band by construction: Δ L\* **21.59** against `{colors.surface-base}`, inside the 21.5–21.7 spread the three destinations hold, so the misregistered crescent has the same headroom they do. Against `dest-trash` #D9B2B6 — the system's only other warm hue — its **separation is 17.21**, clear of the working threshold of 15; it also clears `{colors.icon-mass-neutral}` at 15.63, so the zone marker cannot be mistaken for a utility glyph either. **The separation requirement here is about system coherence, not adjacency.** The Hoja lives on the dispenser card and the rose lives in the 3-Destination Flow; the two never share a surface. But two warm masses at the same lightness that the eye could confuse would make the palette read as careless even when apart. And the Hoja must never be read as a fourth destination.

- **`icon-mass-blue` #9BC1D2** — the system's **active-state hue**: the selected battery's charge in the energy check-in, and the dictating microphone's capsule. **L\* 75.92, chroma 15.42, hue 235.2**, computed with the same verified method, so it is born inside the system band. **This hex is a correction (2026-08-27, Fase 5):** the first issue, #A4BED6 at hue 256, was released with separations measured as hue-angle differences rather than a–b plane distances — the system's own metric; by the correct metric it sat 10.8 from `dest-donate`, under the 15 threshold. Re-issued at hue 235, it clears every chromatic mass: **16.87** from `dest-keep`, **15.67** from `dest-donate`, **28.40** from `dest-trash`, **29.59** from the ochre. The correction is visually near-invisible — both are dusty blue — and purely geometric. **Two pairs sit below 15 by the same metric, accepted with their reasons recorded:** blue vs `{colors.icon-mass-neutral}` at **14.03** (light) and 14.1 (dark), and blue-dark vs keep-dark at 14.15 — these are **states or non-competing meanings** (rest→active on the same mark; a check-in charge vs a destination mass), never simultaneous rivals, each declared redundantly by ink and prose; and the neutral's near-greyness (C 4.6) depresses the distance without perceptual similarity, the chroma gap alone being ~10 units. Blue is the universal *selected/active* hue: it marks a state, not a meaning, with no alarm register and no judgement. **Scope:** state, and state only — it never marks a destination, a meaning or a recommendation. The dark counterpart is `{colors.icon-mass-blue-dark}`.

**Dark mode is in scope and is a separately authored palette — never an inversion.** Inverted dusty pastels either muddy or go neon. The dark ink is a warm off-white (`ink-primary-dark` #ECEAE4, 13.93:1 on `surface-base-dark`, 12.36:1 on `surface-raised-dark`) precisely to warm the cool ground back up; the ground is a deep desaturated neutral, never pure black. **The destinations keep their light form in dark mode — two plates, the global offset, mass under line — in the dark trio's hues.** An earlier decision had them retire to a 6px edge bar plus hairline; it was **overturned by the builder on seeing the bars rendered** (*"quedan muy pobres y con poco contraste"*), and the volume form stands in both modes. The dark mass tier sits at **L\* 62, chroma ~13**, computed by the light trio's own method with the version-C hues: `dest-keep-dark` #7E9C91 (L\* 61.87 · C 12.96 · h 169.7), `dest-donate-dark` #9793AA (61.95 · 13.03 · 298.0), `dest-trash-dark` #AE8E91 (61.99 · 12.88 · 13.3) — **equal weight by construction**: Δ L\* spread 0.12, chroma spread 0.15, minimum hue separation **15.83**, and as masses on `surface-raised-dark` they carry Δ L\* 46 (~5:1) — deliberately stronger than the light system's own ~2:1 mass contrast, which is the volume the bars lacked. The rest of the dark mass tier: `icon-mass-ochre-dark` #A2947E (C 13.5, clearing `dest-trash-dark` at 15.27), `icon-mass-blue-dark` #7B9BA9 (hue 235, the corrected blue), `icon-mass-neutral-dark` #8E9894 (the neutral's own low chroma). `ink-secondary-dark` #99A0A3 is **assigned** as the dark secondary ink — 6.31:1 on base, 5.60:1 on raised, both AA. `accent-soft-dark` #313E3B carries `Hecho` and the chip at 9.27:1 with the off-white ink. The one-time `border-strong-dark` token is **deleted**: nothing needed a strong dark edge that `border-hairline-dark` does not cover. Light/dark follows the system theme (`EXPERIENCE.md`); the dark pass is drawn end to end in [`mockups/dark-mode-1.html`](mockups/dark-mode-1.html).
...
| `{typography.destination-label}` | Lexend | 19sp | 600 | 1.25 | +0.01em | `{colors.ink-primary}` |

**Eight roles now, and the three additions are priced rather than waved through.** The ramp was five, and "five, not one more" held for exactly as long as the only drawn surface was the Dispenser. Drawing the other surfaces — [`mockups/key-screens-1.html`](mockups/key-screens-1.html), which is also where the 200% column lives — broke it in three specific places, each of which was being covered by a role that means something else.
...
- **`{typography.destination-label}`** — the three 3-Destination labels are primary choices, not supporting metadata and not buttons. They need equal, prominent weight without borrowing a button role, so they use 19sp/600 with a 1.25 line-height for the only label that may wrap (`Donar o vender`).
...
**The furniture around the card follows one grammar: mass means work, prose means leaving.** Top-right, the two add-work entries — Cámara and Lápiz at 24px inside `{spacing.touch-target-min}` targets, carrying `{colors.icon-mass-neutral}` under the ordinary treatment. Below the card, the single `{components.ambient-strip}` slot. Bottom-centre, `Nuevo proyecto` — ink-secondary text, the `{typography.action-secondary}` pattern — the one departure from the surface, and deliberately the only control with no pastel mass anywhere on it: **everything carrying colour does something to the day; the way off the surface is prose.** The misregistration idiom is designed to be *seen*, the opposite of discreet, and was never going to mark the quietest control on the screen. The energy check-in lives in the strip, not in permanent furniture — permanent chrome for an occasional action miscommunicates cadence, a defect surfaced and fixed in the canonical render of 2026-08-27, [`mockups/dispenser-canonical-1.html`](mockups/dispenser-canonical-1.html).

**One zone is exempt from the information cap, and the exemption is declared rather than discovered.** The Archetype Template surface inside project genesis **shows a list**, by the builder's explicit decision (E1). It opens from the manual-entry screen behind `Nuevo proyecto` — the photo path needs no surface here at all, because the camera is a direct entry on the Dispenser itself.

**E2 has dissolved — 2026-08-27.** With the camera sited directly on the Dispenser, what sits behind `Nuevo proyecto` is one recommended action (typed project entry) plus one way out (Settings as quiet text): principle 1's letter, satisfied. The multi-path genesis zone exception is retired; the exception register now holds two entries — the PRD's §1.1 dropped E2 upstream on 2026-08-27. The full arithmetic is in `EXPERIENCE.md` § Information Architecture.
...
A full-screen surface: a question, the object, three choices, nothing else. Three rows, each a 64px glyph beside its label in `{typography.destination-label}` — `Quedármelo`, `Donar o vender`, `Tirar o soltar`. Drawn at real size in [`mockups/final-verification-1.html`](mockups/final-verification-1.html) § 5, where the ink parity of the row (§ 2) was measured.
...
- The trio seed reads at rest — now trivially. History, kept readable: this row once barred the five motion arcs on measurement — they left the glyph's own area, collided with the neighbouring label, and contrast-weighted pushed the seed to 122% of the box, the heaviest in a row that must be equal — a bar rather than a size floor, since the 64px here would otherwise have cleared the 56px threshold. **Dissolved 2026-08-28:** the dash lever is gone outright (see `{components.seed-glyph}`), so no live bar remains — the seed is at rest here for the same reason it is at rest everywhere.
...
### `destination-mark-dark`
In dark mode the three destinations keep **the light form unchanged**: two plates, the global offset, mass under line — the line in `{colors.ink-primary-dark}`, the mass in `{colors.dest-keep-dark}` / `{colors.dest-donate-dark}` / `{colors.dest-trash-dark}`. The early bar-plus-hairline construction was overturned by the builder on sight (*"quedan muy pobres y con poco contraste"*); the volume form stands in both modes, and the full accounting lives in Colors.

### `icon-glyph` — the treatment, in full
Ten glyphs: **Cámara, Álbum, Caja, Bolsa, Reloj, Hoja, Lápiz**, the **dandelion seed**, the **batería**, and the **Micrófono capsule**. Caja / Bolsa / seed are the destination trio; Hoja is the FlyLady zone marker and carries `{colors.icon-mass-ochre}`; the batería is the energy check-in's mark and the Micrófono is Manual Capture's dictation affordance (both registered — see the rule below); the rest are utility, carrying `{colors.icon-mass-neutral}`. **The Ajustes glyph is dissolved** — Settings is reached as quiet text, so no drawing ever needs to read as "settings", and the gear question dies unanswered by removal rather than by exception.
...
- **Stroke: `stroke-width(u) = target_px × 24 / render_px`, targeting 1.5px at 24px.** Computed values: 1.80@20 · **1.50@24** · 1.00@36 · 0.90@40 · 0.75@48 · 0.5625@64 · 0.50@72 · 0.375@96. **1px was tested and is unrecoverable**: against L\* 76 masses the mass wins the attention split (the glyph reads as a pastel silhouette with a dirty edge instead of line art over a second plate) and the 8% offset degrades from misregistration into antialiasing. All three recovery routes cost more than accepting 1.5.
...
- **A mass whose axis contradicts the global vector — or that sits inside a closed container — is REGISTERED.** This is the general rule; two glyphs live under it, each on both grounds. The **batería**: a horizontal casing against the 45° vector, and a charge inside a closed container where the misregistration would read as a *leak*. The **Micrófono capsule**: a vertical axis against the vector, and a capsule is itself the container. Axial glyphs — the Lápiz, the seed, the destination trio — keep the misregistration. The rule is checkable per glyph by asking one question: does this mass's own axis coincide with the global vector?
...
- **Implementation check:** the circle's radius is the one number in this system settled at draw time rather than by measurement. It is settled against the coverage rule above, not by a token; Story 1.2 cannot complete without recording the chosen value.
...
- **The comb, not the feather, is intended — and the accepted trade is twice as large as the record said.** At 96px the specified glyph's mean tip-to-tip gap is **13.5px** against 1.5px lines, roughly 1:9 ink-to-air at the broad edge: the pappus reads as a **comb**, not a feather. That 13.5px figure is correct and stands. What was wrong is the reference it was compared against. **The reference in `imports/dandelion-seed-reference.png` carries 17 filaments, not the ~12 recorded throughout this session** — measured by ring-sampling the ink mask about the hub, 18 crossings of which one is the stem, at four separate radii. Scaling the same measured tip gap inversely with count puts the **reference-equivalent gap at ~6.4px at 96px**, not the 13.5px the spec quoted as the reference's own figure. So the specified glyph puts roughly **twice** as much air between tips as the reference does, not the ~55% more that "12" implied, and the comb-not-feather consequence is about twice as pronounced as recorded. **Stated plainly because it matters who accepted what:** the builder accepted this trade on the wrong number. The decision itself does not change — 8 filaments were chosen on legibility cues, never as a ratio of the reference, and `Nunca añadir rasgos` is his own rule — but the trade he accepted is larger than it looked. The more graphic, stamp-like drawing suits the print register; that is the position, now held against the right figure. The only palliative compatible with "one drawing, scaled" was the motion strokes — dissolved with the dash lever on 2026-08-28 (see the dissolution record below); the comb trade stands unpalliated.
- **Motion dashes: dissolved — the seed never draws them.** History, kept readable: the arcs were measured to help at **56px and above** and to hurt at **36px and below** (4px specks read as dirt and cost 19.4% extra ink for nothing); from 2026-08-26 the lever carried a 56px threshold — the `components.seed-glyph.motion-dashes` token, since deleted — with dashes-on reserved to the illustration register (Warm Return was to draw them at 168px) and the destination trio at rest as a named, accepted consequence. **Dissolved 2026-08-28:** the builder removed the lever end-to-end — no threshold, no flag, no register and no surface may draw the arcs. What the lever's only pinned rendering showed — the standalone 64px golden's movement lines — was carried by no other pinned Flutter golden (the mockup page's dashes-on render is history, superseded per its own note), and no shipped surface had ever drawn them on; dissolving the lever changed exactly that one golden, re-pinned at rest. The renegotiation record is the Story 1-2 amendment spec, `_bmad-output/implementation-artifacts/spec-1-2-remove-seed-motion-dashes.md`, cited in the house style of "Paid into the PRD". The lever's old inverse-rule finding — dashes off turns the same drawing from *arriving* into *at rest* — is now the one drawing's permanent state: **at rest at every size, in every register, on every surface.**
...
### `zone-marker` — Hoja
The FlyLady zone marker, and the reason both open glyph slots closed with no new drawing: the seed took the destination slot, so the Hoja stayed free. Rendered quiet — `{spacing.glyph-zone-marker}` in `{colors.ink-secondary}`, label in `{typography.support}`, sitting at the foot of the dispenser card. At that size it is a place-marker, not a glyph to be read. **Its mass is `{colors.icon-mass-ochre}` — ochre**, computed rather than chosen (see Colors), and neither the neutral utility mass nor any destination hue, so the marker can be read as neither a destination nor a utility control. Its 24px crescent must be verified on a real handset in light and dark mode before Story 1.8 completes.

### `energy-checkin` — the battery marks
Three batteries at 24px inside `{spacing.touch-target-min}` taps: **llena / media / baja**. Each is a casing and nub drawn line-only, with the charge as a **registered mass filling from the left** — widths 12.8u / 6.4u / 3.2u in the 24 viewBox. The charge is the state and the fill level is the meaning: a battery's charge is a *quantity*, culturally unjudgeable — which is why the form was chosen over both a traffic light (positivo-neutro-negativo, the shame register) and partial-fill circles (which read as rating scales). **Selected: charge `{colors.icon-mass-blue}` + casing `{colors.ink-primary}`. Unselected: charge `{colors.icon-mass-neutral}` + casing `{colors.ink-secondary}`.** Llena is pre-marked as the standing default. The charge carries **no offset** — a documented exception on two grounds (leak inside a closed container; transverse axis), recorded under `{components.icon-glyph}`. The check-in lives in `{components.ambient-strip}` once a day; there is no permanent energy furniture anywhere.

### `ambient-strip` — six residents, one spec
A single strip below `{components.dispenser-card}`: sentence in `{typography.support}` and `{colors.ink-secondary}`, tappable where an accept action exists, ✕ dismissal at `{spacing.touch-target-min}`, **at most one resident visible at a time**. The chrome rule, decided with the strip: **ephemeral = bare, persistent = hairline** — the daily energy check-in, the three suggestions and the one-time first-run curation offer sit bare on the ground; the Sunday self-report carries a 1px `{colors.border-hairline}` edge because it stays until answered. The self-report's answer row is **numeric 1–5** with end labels `Nada` / `Muchísimo` — numbers chosen for zero ambiguity after five empty circles failed to read as anything at all.

### `curation-row` — one component, three homes
A platform switch row: **cluster name, cadence in `{typography.support}`, switch** — tappable anywhere in the row. The three homes are the E1 template surface, onboarding's one-time strip offer, and Settings' sub-screen; all three render the identical row, so curation looks like itself everywhere it is found. **Cadence carries the row's only description** (diaria / semanal / mensual-estacional): rhythm is product metadata, not volume — FR-31 forbids counting tasks, and cadence counts nothing. Switching produces no visual consequence beyond the switch itself: no count, no summary, no copy about what changed, and never a task-level row.

### `microphone-glyph` — the dictation affordance
The universal dictation form: a vertical capsule, stem and U-arc stand in `{colors.ink-primary}`, with the capsule as a **registered mass** — `{colors.icon-mass-neutral}` at rest, `{colors.icon-mass-blue}` while dictating. 24px inside a `{spacing.touch-target-min}` tap, at the one-line field's end on Manual Capture and nowhere else; placement does half the *press-to-speak* work, the form does the rest. Drawn with its rest and dictating states, in the field's current layout, in [`mockups/mic-manual-capture-1.html`](mockups/mic-manual-capture-1.html). The dictating state is declared in **ink and prose** — the blue capsule plus the caption `Escuchando…` in `{typography.support}` — and **never by motion**: a breathing cue was offered and declined, state being ink + prose in this system. Where on-device recognition is unavailable the glyph is simply absent — never greyed, never explained.

**The choice this glyph records:** the diagonal handheld mic was fully legal under the offset (45° axis, the Lápiz precedent, zero exceptions) and lost anyway — the capsule is what every phone already taught its users, and recognition beat rule-purity. The honest price was paid in the rule, not hidden in the drawing: the registered-mass rule **generalised** to cover every axis-contradicting or contained mass, rather than this becoming a one-off exception.

### `size-option` — Manual Capture's three-size picker
The only selector **attached to a capture** — the energy check-in also selects, but it filters the pool rather than writing a value into something captured, so the original "only selector" claim is narrowed rather than broken. The states did not exist: `{components.duration-chip}` was specified as a *display*, so there was no selected and no unselected. **Selected is `{colors.accent-soft}`** — the chip's own fill, borrowed because the options are durations and `accent-soft` is already the fill of the surface that carries a duration. **Unselected is `{colors.surface-raised}` with a 1px `{colors.border-hairline}` edge.** Both states keep `{colors.ink-primary}` and `{typography.duration}`, both are `{rounded.full}` (see Shapes), both hold `{spacing.touch-target-min}`, and they sit `{spacing.action-gap}` apart. **No option ever carries a glyph**, so the no-glyph-inside-`accent-soft` rule survives intact — which is exactly why the selected state can use that fill at all. In dark mode the selected option takes `{colors.accent-soft-dark}`.
...
Two of these surfaces are drawn at register scale in [`mockups/seed-at-scale-1.html`](mockups/seed-at-scale-1.html) § 4 — a page that drew Warm Return at 168px with motion dashes and the permission-to-rest screen at 150px without them, the origin of the dashes-on / dashes-off reading. That rendering is history twice over — the page also draws the seed with twelve filaments — superseded, per the note at the top of the page; and since the 2026-08-28 dissolution both surfaces draw the seed at rest (see `{components.seed-glyph}`).
...
| Treat "no red" as *no alarm register* — a dusty rose at L\* 76 / C 15 is admitted | Use red as alarm, warning iconography, or exclamation marks outside the fixed completion acknowledgement `¡Buen trabajo!` |
...
| Draw masses **registered** when the glyph's axis contradicts the 45° vector or the mass sits in a closed container (batería, Micrófono) | Misregister a transverse or contained mass — it reads as a leak |
| Keep `{colors.icon-mass-blue}` for **state** — selected charge, dictating capsule — and nothing else | Give blue a meaning (a destination, a recommendation), or any selected battery a rose charge |
| Declare the dictating state in ink and prose: blue mass + `Escuchando…` | Animate the mic — the breathing cue was offered and declined; state is never motion |
| On the Dispenser, let **mass mean work and prose mean leaving** | Give the quiet departure a glyph — the offset is designed to be seen, the opposite of discreet |
| Strip chrome: **ephemeral bare, persistent hairline** | A box around a suggestion, or a naked persistent self-report |
| Author the check-in as numeric 1–5 with `Nada` / `Muchísimo` ends — zero ambiguity | Bare dots or unlabelled scales — a scale without anchors is a riddle |
| Let a curation row carry **cadence** — rhythm, not volume | Count tasks in a curation row, summarize what a disabled cluster held, or narrate the switch |
...
| Never draw motion dashes — the lever was dissolved 2026-08-28; the seed is at rest at every size, register and surface | Reintroduce them at any size, register or surface |
...
| Reflow `{components.dashboard-highlight-row}` to one column when a caption passes two lines | Shrink the grid, scale the dp gaps, or let the three-column row survive unreflowed at 200% |
...
**Two exceptions are declared in this system, and no third is implied.** Each names the invariant it departs from. The **FR-23 dashboard** departs on information density. The **per-scan consent gate** (`{components.action-equal-pair}`) departs on the one-recommended-action rule, which it carries at zero. The genesis zone's E2 — the third exception until 2026-08-27 — **dissolved** when the camera moved to the Dispenser and the zone behind `Nuevo proyecto` came back to one recommended action plus one way out. Anything claiming a new exception is a defect.
...
1. **The second locale.** The PRD's FR-9 says no interrupted, incomplete or overdue state is displayed *"in either language"*. The second language is never named. Its script and typical string length bear on the 200% fold and on the type ramp.
2. **Where the completed card goes.** The card leaves rather than changing content (Android's gesture vocabulary governs, and the app is Android-only). That it must exit the screen entirely and never fly toward a counter, pile or badge — and whether its departure runs along the global 45° offset axis, making the departure itself the celebration — is proposed and unconfirmed.
3. **System chroma.** Version C raises chroma across the whole system from ~10 to ~15, not only the destination row, and Aliento was chosen for being airy. Whether that reads as too present has not been judged in situ; the lever, if so, is all three down together to ~13. `{colors.icon-mass-ochre}` and `{colors.icon-mass-blue}` now sit in the same chroma band, so they move with them if the lever is ever pulled — never alone.
...
- **Seed-glyph pompom radius:** Story 1.2 records the exact drawn radius, which must remain larger than 1.70u, displaced toward the dense side and wholly covered by filaments at its free edge.
- **Zone-marker readability:** Story 1.8 verifies the 24px Hoja on a real handset in light and dark mode before completion.
