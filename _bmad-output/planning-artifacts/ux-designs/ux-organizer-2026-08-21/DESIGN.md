...
**Both amendments — the template list, and the multi-path genesis — must be flagged back upstream to the PRD.** The PRD writes principle 1 as an invariant that overrides individual FRs, and states that the user never chooses from a list. Leaving either unflagged would make the two documents diverge silently instead of by decision; it is an upstream edit owed, not a local preference. The full arithmetic is in `EXPERIENCE.md` § Information Architecture.
...
**`Tirar o soltar` is a confirmed, intentional change of string.** The PRD fixed this destination as *Trash-Recycle* / `Tirar o reciclar`; `Tirar o soltar` is deliberate, because *soltar* dodges both the deletion vocabulary of a bin and the compliance vocabulary of recycling arrows — the register the whole trio depends on, since the third choice must not read as the bad one. **This is a string change that must be flagged back upstream to the PRD.** The PRD fixed that destination name, so leaving it unflagged would make the two documents diverge silently instead of by decision; it is an upstream edit owed, not a local preference.
...
4. **Dark-mode destination hues.** `{colors.dest-keep-dark}`, `{colors.dest-donate-dark}` and `{colors.dest-trash-dark}` were hand-authored against the *superseded* light trio, when the third slot was a light sand rather than a dusty rose. They have not been re-authored against version C, and their equal-weight parity as 6px bars has not been measured. There is no dark counterpart to `{colors.icon-mass-ochre}` either.
