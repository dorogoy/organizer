...
7. **Whether the Hoja's ochre crescent reads at 24px under a secondary-ink line.** Surfaced by resolving the Hoja's mass, and not answerable from what exists: the two-tier system's Δ L\* 18–20 threshold was measured for a mass sitting under an `{colors.ink-primary}` line, and the zone marker's line is `{colors.ink-secondary}` at L\* 41.48, rendered at `{spacing.glyph-zone-marker}`. The mass clears the threshold against the *ground* (Δ L\* 21.59), but the line-to-mass relationship the threshold was calibrated against is not the one on this glyph. The Hoja's axial extent at 24px has also not been measured against the 8u minimum. Nothing here should be assumed to pass by inheritance. **The dark sibling carries the same question** — `{colors.ink-secondary-dark}` over `{colors.icon-mass-ochre-dark}` at 24px — and both verify on a device.
