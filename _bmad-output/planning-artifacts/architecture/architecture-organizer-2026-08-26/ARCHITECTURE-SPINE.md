...
- **Rule:** One `Calendar` in the core is the **only** thing that converts an instant to a period. Three periods exist and no more:
  ...
  - **Week** — seven domestic days anchored on **Monday**, so FR-31's zone rotation turns on a boundary the user does not notice; SM-2's *"from Sunday onward, persisting until answered that week"* is the week's last day, and one answer closes it. The report window is **that Sunday only**: an unanswered report expires at the week boundary and never carries into the next week.
...
- **Binds:** FR-11, FR-12, FR-29, FR-31, AD-3, AD-15, AD-23
...
| Cross-cutting — i18n | ARB through `flutter_localizations` / `gen_l10n`, Spanish the only shipped locale, every key present, no key resolved from a computed name (AD-15). |
...
| Gemma 4 E4B (Local path, deferred) | **3.66 GB** on disk | 3654 MB Android model size, ~3283 MB peak memory, per the official model card — not the ~2.5 GB the first draft claimed, and understated in the direction the storage constraint cares about |
...
- **OQ-1's deployment topology.** Open by the PRD's own ruling; AD-9 keeps it a configuration rather than a rewrite. Two corrections to the first draft's cost estimate, in opposite directions: the model is **3.66 GB**, not 2.5, and **`flutter_gemma` 1.6.5 exists** with MediaPipe and LiteRT-LM engines and Gemma 4 E4B listed — so the Local path is not the hand-written platform channel this spine first claimed. Revisit with the real-photo tests, against the standing constraint that not every handset can host a model nor every user spare the storage.
