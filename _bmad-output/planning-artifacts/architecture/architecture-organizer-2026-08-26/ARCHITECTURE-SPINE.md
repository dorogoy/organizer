...
| `http` | 1.6.0 | The BYOK Slicer's transport (story 4-4, AD-7). Pure Dart, so the resolved Gradle graph is unchanged; the import is legal only inside `lib/egress/`, which the Dart import seal carries, and INTERNET-in-main is the manifest seal's deliberate 4-4 edit. Exact-pinned like every runtime dep |
