...
- **Rule:** `lib/l10n/app_es.arb` is the single table; `lib/strings/` holds no literals and exists only as generated accessors; a lint fails on any string literal reaching a widget. No interpolation that produces a sentence — a number substituted into a fixed sentence is permitted, a sentence assembled from fragments is not; ICU plural and number placeholders are the one controlled exception. **The SM-C2 audit list is every key in the shipped ARB, minus an explicit reviewed exclusion list**, so silence adds a string to the audit rather than omitting one. FR-29's seven no-Slicer strings are pinned **by key** in a build check that requires both a non-placeholder value **and** a reviewer sign-off marker in the ARB metadata — existence and review are separate gates.
...
| Environment — devbox | Every toolchain command runs inside `devbox shell` (CI: `devbox run --`), never from the host. `devbox.json` and `devbox.lock` are committed; a story that adds a toolchain need adds it to `devbox.json` and re-commits the lock in the same pass. `devbox add flutter` is banned — it would silently move the Flutter pin (NFR21). |
...
| dev environment | devbox 0.18 | `devbox.json` + committed `devbox.lock` at the root own the toolchain (Story 1.1, NFR21). JDK 17 = nixpkgs `jdk17`. Flutter 3.47.1 is **not** the nixpkgs `flutter` package — nixpkgs tops out at 3.47.0 (`devbox search flutter`, 2026-08-27) and the pin does not move — it is the official stable tarball, sha256-pinned, fetched by the devbox bootstrap |
| Gemma 4 E2B (Local path, deferred — **preferred candidate**) | Android size **not yet recorded** | Tested before E4B: smaller on exactly the axis the Deferred section's standing constraint cares about (handset class and spare storage). No figure is asserted here — it is read off the model card when OQ-1's test runs |
| Gemma 4 E4B (Local path, deferred — fallback if E2B's slices are too weak) | **3.66 GB** on disk | 3654 MB Android model size, ~3283 MB peak memory, per the official model card — not the ~2.5 GB the first draft claimed, and understated in the direction the storage constraint cares about. These figures are E4B's and say nothing about E2B |
...
- **OQ-1's deployment topology.** Open by the PRD's own ruling; AD-9 keeps it a configuration rather than a rewrite. Two corrections to the first draft's cost estimate, in opposite directions: E4B is **3.66 GB**, not 2.5, and **`flutter_gemma` 1.6.5 exists** with MediaPipe and LiteRT-LM engines and Gemma 4 E4B listed — so the Local path is not the hand-written platform channel this spine first claimed. **The slicing-quality question is settled on the development machine first, through Lemonade** — a local inference server with an OpenAI-compatible endpoint — with **E2B tried before E4B**, as a cheap kill filter rather than on the validation handset. It is a valid venue because slicing quality is a property of the weights and the prompt, not of the host; it is only a filter because the Android artifact is differently — more aggressively — quantized, so a desktop **failure kills a candidate outright** while a desktop **pass is provisional** and must be re-verified on the handset before commitment. Storage, peak memory, inference latency and thermal behaviour are handset questions and stay deferred here. Revisit with the real-photo tests, against the standing constraint that not every handset can host a model nor every user spare the storage.
