...
- **Ambient Invitation** — The single optional daily notification: silent, content-free, at a user-chosen hour — an invitation, never a recalled debt (FR-24).
...
- Tapping 🔴 mid-task behaves like the checkpoint (FR-10): the card in progress can be finished; the filter applies to the next deal, never to the active card. Work in progress is never withdrawn without comment.
...
- Genesis by typed or form description (FR-11) sends that text to the provider, and the send action is the consent: the genesis screen states in plain Spanish what is sent and to which provider, on the same terms as the scan confirmation — an explicit send with the destination named, not a separate dialog (§7).
...
- The catalogue is curated at **cluster** level — zones and upkeep groups — never at task level. No screen anywhere enumerates individual catalogue entries: a browsable 80-item catalogue is the task list NL-1 abolishes, arriving through the template door.
...
- Curation is reachable from onboarding and settings only, never from the Dispenser (§7). A disabled cluster's tasks simply never appear; disabling one produces no count, no summary and no copy about what was turned off. Onboarding abandoned mid-curation leaves the default standing — every cluster active — because onboarding curation is inherently optional and the first composed day is never empty.
...
- **Accessibility.** Legible at 200% system font scale with no truncation; every action reachable one-handed; haptics never the sole completion signal. **No keyboard is required on any user surface.** The photo path needs no words and Manual Capture's one line can be dictated (FR-32), so someone who cannot type comfortably on a touch screen — small keys, motor difficulty, poor eyesight — is never shut out of the only surface where the app asks for writing. Settings is not that surface: it is the validator's (see the settings/Dispenser split above) and the BYOK key entry lives there. This is a floor on the whole build rather than a property of FR-32: any future user surface that demands typed input breaks it.
...
- **Background minimalism.** The only background work is FR-24's daily invitation. No sync, no location, no persistent service, no wake locks, **and no microphone outside an explicit press** (FR-32). Voice adds the build's one new runtime permission, requested at first dictation and never at first run. The automatic export (FR-30) is deliberately *not* an exception: it runs in the foreground at natural moments — session end, app backgrounding — because a periodic sync job would buy durability with the one property this bullet protects.
...
| On-device candidate | Gemma 4 E4B (Android, native multimodal) — exists; slicing quality unproven | OQ1, §4.4 |
...
| Microphone when unavailable | absent and silent — no error, no greyed state, no install offer; permission at first use, never at first run | FR-32, OQ-13 |
| Keyboard dependency | none on user surfaces — every surface asking the user for writing has a wordless or spoken route; settings (incl. the BYOK key) may type | §7, FR-32 |
