...
- **Functional:** Capture a messy space (photo) and get an accurate, effort-estimated micro-plan without typing or categorizing. The photo path is the typeless one; Manual Capture (§4.9) is a floor for what the app cannot see, not the main road.
...
- Power users wanting GTD density: projects hierarchies, tags, filters, kanban, calendar writes. Manual Capture (§4.9) is the one typed entrance and it is deliberately crippled — no list, no editing, no browsing — precisely so this group stays a non-user.
...
- **UJ-5. Sergio anota algo a mano en diez segundos.**
  Something occurs to Sergio that the app cannot know and no photo can reveal: he promised to water the neighbour's plants on Thursday. He taps Manual Capture from the Dispenser, types one line, picks "3 min" from three sizes, and it is out of his head. No project, no category, no date, no confirmation screen. Days later it surfaces as an ordinary dealt card, indistinguishable from a sliced one. He never sees a list of what he has captured, because there is no list. On the day his API key expires, this is the only path still working — and the app keeps being useful without saying a word about it.
...
- **Floating Time Bag** — The daily flexible minutes quota the user commits to, spent on demand instead of fixed appointments. Values: §10.1.
...
- **Manual Capture** — The floor of task genesis: the user writes one Micro-task and picks its size from three options that are the 1-3-5 taxonomy (30 s / 3 min / 10–15 min). Available always, with or without AI, network, key or account. Never a list, never browsable, never editable once left. Details: FR-27.
...
- **3-Destination Flow** — Item triage: Keep / Donate-Sell / Trash-Recycle.
...
| 27 | Manual Capture floor: one task, three sizes, no list | 4.9 Manual Capture |
...
**Description:** Dual-lifecycle project structure (Evergreen + Epic) automatically woven into each day's 1-3-5 composition within the Time Bag. Epic Projects carry Invisible Buffers; deferrals and absences are absorbed by the Silent Rescheduler; dormant Epics may be gently suggested by seasonal triggers. The user never manages folders or triages categories. Realizes UJ-1, UJ-2, UJ-3.

**Functional Requirements:**

#### FR-11: Dual-lifecycle projects with archetype templates
The user can instantiate Evergreen Projects from pre-sliced Archetype Templates (5 default FlyLady zones + daily anchors), and create Epic Projects from a photo or from a typed/form description — the latter always through the Slicer, never from a template.

**Consequences (testable):**
- Fresh install reaches a working 1-3-5 day with zero typing, zero network, zero key and zero model: the Evergreen zones and daily anchors ship pre-sliced (§3). This is product content, honestly labelled as such, not a Slicer substitute.
...
**Consequences (testable):**
- Composition respects budget: sum of estimated durations ≤ Time Bag.
...
During purge steps, the app presents guilt-free detachment questions — the factual one ("Have you used this in the past 12 months?") and the one that does the real work ("Does this deserve your physical and mental space?") — alongside the Keep / Donate-Sell / Trash-Recycle triage.
...
**Description:** The one typed entrance, and deliberately the poorest one. Something occurs to the user that the app cannot know and no photo can reveal — a favour promised to a neighbour, a form to hand in. Manual Capture takes one line and one of three sizes, then returns to the Dispenser. It exists so the app is never useless: with no key, no network, no model and no account, it still holds work and dispenses it. It is a floor, not a road — every affordance that would make it comfortable to live in has been left out on purpose. Realizes UJ-5.
...
- **Voice-first natural-language slicer** — Phase 3.
...
- Dual-lifecycle projects, Archetype Templates (5 FlyLady zones), 1-3-5 weaving, Invisible Buffer, Silent Rescheduler, seasonal suggestions (FR-11–FR-15)
...
- **Offline by default — execution, not genesis.** Dispensing, completing, weaving, rescheduling, the album, the dashboard, export and Manual Capture work with no network, always. Slicing does not: on the BYOK path every slice and re-slice needs the network, on the Local path none does. The shipped Evergreen day is exempt because it was sliced at build time rather than at runtime (FR-11) — a phone in airplane mode on day one still has zones, anchors and a composition. The distinction is load-bearing (§1.1) — an offline app executes, it does not beget. Airplane mode is supported, never an error state; a slice or rescue that cannot reach its provider degrades per FR-29 and queues nothing.
...
- **Accessibility.** Legible at 200% system font scale with no truncation; every action reachable one-handed; haptics never the sole completion signal.
...
- **Background minimalism.** The only background work is FR-24's daily invitation. No sync, no location, no persistent service, no wake locks. The automatic export (FR-30) is deliberately *not* an exception: it runs in the foreground at natural moments — session end, app backgrounding — because a periodic sync job would buy durability with the one property this bullet protects.
...
3. Content of the 5 default FlyLady zone Archetype Templates (which rooms/routines ship as defaults), plus the per-person selection at install: each user curates their own zone set — no household-level assignment, since projects are personal portfolios. Settled 2026-08-21, then refined the same day by a second-order pass: the split is by **lifecycle, not by mechanism**. Evergreen templates ship **pre-sliced** — product content authored at build time, identical for every user because FlyLady is published, and free of network, key, model and first-run cost. Epic material is **never templated**: it is personal to this user's home, so only the Slicer authors it. The first formulation (everything a Slicer seed) was rejected because it made the first thing a new user does cost money and require the network, immediately after §7 had celebrated that the app needs neither. What remains open is only the wording of the five zones. → Sergio, before FR-11 is built.
...
| Manual Capture | always available; one Micro-task per capture, three sizes (30 s / 3 min / 10–15 min); no list, no editing once the capture surface is left | FR-27 |
...
- §4.3 FR-11 — exactly 5 default FlyLady zone templates; their concrete content is Open Question 3.
