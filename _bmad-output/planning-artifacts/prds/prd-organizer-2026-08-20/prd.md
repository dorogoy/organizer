...
**Revision 2026-08-26 (coherence review).** A `bmad-review` coherence pass forced the Focus Chunk's arithmetic to close. The FR-31 coverage floor now counts *eligible* units — distinct 10–15 min entries across weekly zones and the `fondo` cluster, which becomes a secondary source when a zone's week runs thin — because the earlier ≥45 count included 3-minute entries that can never occupy the slot. Zone rotation runs over active clusters, so curation never leaves a day zoneless. The Time Bag keeps its 5–30 range and gains its limit case: below 10 minutes the day composes without a "1", silently. The Anti-Marathon Checkpoint became recurrent, and sessions shorter than one interval are exempt — their close already is permission to stop. Manual Capture's line is ruled plain text that models nothing, not even a day written in it, and UJ-5 stopped exhibiting a date. Genesis text joined the egress map as the third payload with send-as-consent, rescue steps inherit the original task's origin, and FR-12's stale "always exceeded" arithmetic is now past tense. FR-4, FR-5, FR-7, FR-8, FR-10, FR-11, FR-12, FR-25, FR-26, FR-27, FR-31, §3, §7, §9 and §10 carry the consequences; findings and triage in `review-coherencia-2026-08-26.md`.
...
- **Focus Chunk** — The "1" of the 1-3-5 composition: a 10–15 min Micro-task drawn from the active Epic Project, the active weekly zone, or the `fondo` cluster filling in (FR-31).
...
- **Anti-Marathon Checkpoint** — The recurring rest offer inside a session, at every multiple of the interval (default 15 min): explicit permission to stop, never a wall — continuing past it is exclusively the user's active choice. Sessions shorter than one interval end without one; their close is the rest. Values: §10.1.
...
- With a Time Bag below 10 minutes no Focus Chunk fits, and the day composes without one — the "1" is absent, silently, with no debt and no mention: the same mechanism as a 🔴 day (FR-4, UJ-4). The budget buys what it can hold (added 2026-08-26).
...
A session offers rest at every multiple of the checkpoint interval (default 15 min, configurable 10–15) with an explicit permission-to-stop screen. The checkpoint is a rest offer, not a wall: continuing past it is exclusively a user-initiated action. Sessions shorter than one interval simply end, and their close is the permission to stop — an offer a short pocket cannot hold is never manufactured. Reworked 2026-08-20 (superseding the same-day hard-cap ruling) and made recurrent 2026-08-26: the app guides, the user decides — session length included.

**Consequences (testable):**
- At the checkpoint the current Micro-task can be finished; the permission-to-rest screen is the primary surface.
- The offer repeats at each interval boundary in sessions longer than one interval: a 45-minute pocket with a 15-minute checkpoint offers rest three times. A session shorter than the interval reaches no checkpoint — its close (FR-8) already is the permission to stop, and the earlier "every session reaches a checkpoint" was arithmetically false for 5–9 minute pockets (corrected 2026-08-26). When the final checkpoint coincides with the close, that close is the offer (UJ-1).
...
- Exactly one FlyLady Zone is active per day, rotating weekly over the *active* clusters: the week of a disabled zone passes to the next active zone, so curation never leaves a day without an active zone (FR-31; ruled 2026-08-26).
...
- The canonical 1-3-5 sums to roughly 26 min (≈15 + 9 + 2.5). Before the advance/upkeep split (2026-08-23) that sum always exceeded the 15-minute default Time Bag, and "scaled to the current Time Bag" hid the arithmetic; under the split only the Focus Chunk is charged, and 15 against 15 exceeds nothing. Scaling drops upkeep and habits by count, never by shrinking a Micro-task's own estimate.
...
- Genesis by typed or form description (FR-11) sends that text to the provider, and the send action is the consent: the genesis screen states in plain Spanish what is sent and to which provider, on the same terms as the scan confirmation — an explicit send with the destination named, not a separate dialog (§7; added 2026-08-26).
...
- The line is plain text, and the app models nothing written in it — including a day. A line that says "el jueves" is not held until Thursday: the 3-day deal window runs from capture regardless, no exemption exists, and nothing is ever parsed out of the text (FR-32). A capture with a real deadline does not belong on this surface (ruled 2026-08-26; OQ-11 carries the copy question).
...
- **Coverage floor, in eligible units:** the pool eligible for the Focus Chunk slot — weekly zone routines and monthly/seasonal upkeep at 10–15 min, across all zones and clusters — holds at least 28 distinct entries, so 28 dealt Focus Chunks never repeat a Micro-task even with no Epic Project active. Only 10–15 min entries count towards the floor: a 3-minute entry can never occupy the slot, so counting it verified nothing (corrected 2026-08-26 — the earlier ≥45 floor included ineligible units). Daily items recur by definition and are excluded; everything else must have the breadth to keep a rotation from collapsing into the same handful.
- **The floor counts dealt Focus Chunks, not calendar days.** A 🔴 day deals no Focus Chunk at all (FR-4) and consumes nothing of the rotation; the repetition of daily anchors on such days is their nature, not a failure of this floor (UJ-4).
...
| Anti-Marathon checkpoint | default 15 min, configurable 10–15 — a rest offer at every interval multiple, never a session limit; sessions shorter than one interval simply end | FR-10 |
...
| Evergreen Library | three cadences (daily anchors + Baseline Upkeep / weekly zone routines / monthly-seasonal), pre-sliced at build time; coverage floor in eligible units — ≥ 28 distinct 10–15 min non-daily entries across weekly zones + `fondo`, so 28 dealt Focus Chunks never repeat | FR-31, A12 |
