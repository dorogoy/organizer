...
- Genesis by typed or form description (FR-11) sends that text to the provider, and the send action is the consent: the genesis surface states in plain Spanish what is sent and to which provider, on the same terms as the scan confirmation — an explicit send with the destination named, not a separate dialog (§7).
