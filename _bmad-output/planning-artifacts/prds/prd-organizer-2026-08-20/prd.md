...
10. ~~Which providers pass the FR-28 allowlist gate, and what the app does when a listed provider changes those terms after ship~~ — closed 2026-08-26 at `bmad-architecture`. The gate narrowed to **written no-training** (retention dropped — FR-28 owns the reasoning). Initial list: Google Gemini (paid API), OpenAI, Anthropic — all three exclude API data from training by written terms, verified 2026-08-26. Cost is not a selection criterion: a room photo prices at roughly 0.003–0.006 USD per scan, so A8's target is met by every candidate with room to spare, and selection rests on the terms gate and structured-output reliability alone. Stale-terms behaviour: nothing automatic — the dated statement FR-28 already requires is the whole mechanism, and a build is what corrects it. **Extended 2026-09-03 — OpenRouter admitted as the fourth allowlist entry, permitted without reservation.** Sergio's ruling ("permitido sin duda alguna") closes the verification OQ-1's closure left pending, on a written basis verified live 2026-09-03. OpenRouter's own handling: any prompt retention is opt-in, and OpenRouter has never shared, sold or licensed prompt data to a third party (its data-collection documentation). The upstream half — the reason an aggregator needed its own look — the app enforces itself, per request: `provider.zdr: true` restricts routing to zero-data-retention endpoints, and OpenRouter states that providers which do not retain data cannot train on it; endpoints without an ascertainable policy are marked retain-and-train by default, so the exclusion errs safe. Request-level ZDR can strengthen but never weaken account settings, so the guarantee does not depend on what the user has toggled. This entry is stricter than the gate requires (retention stays deliberately ungated) and is the first whose no-training property is enforced technically rather than by written terms alone. FR-28's entry mechanics apply unchanged — provider name and the 2026-09-03 verification date, corrected only by shipping a build; the model ids it carries are fixed when story 4-2 wires the egress, and they must sit in OpenRouter's ZDR endpoint list (`api/v1/endpoints/zdr`), the same frozen-at-build discipline as the rest of the allowlist.
...
- **2026-09-02 (OQ-1 closed — model-evaluation harness, story 4-1)** — the five-candidate harness over 10 real photos (10 space types, one prompt, one schema, one pre-confirmed 8-of-10 bar, one request per photo) settled OQ-1: E2B 3/10 and E4B 2/10, both killed outright on desktop per the one-direction rule (E2B fenced all 10 responses — the single-fence strip is a dated, per-photo-recorded bar amendment); cloud trio via OpenRouter: gemini 9/10 selected over openai 9/10 (cascade tie-break) and anthropic 6/10. Ruling: Smart Slicer ships cloud BYOK with gemini via the provider's direct API; Local stays the debug-only canned stub (FR-28, AD-9); the deferred handset questions die with the local path. OpenRouter admitted as a candidate fourth provider for the access path, pending FR-28's written no-training terms gate. Run cost on record: gemini $0.00757/10 photos. Evidence: `eval/results/report.md` + `scores.json` + `costs.json`.
- **2026-09-03 (OpenRouter permitted — the fourth provider, no doubt)** — the verification OQ-1's closure left pending is done and the ruling is unambiguous (Sergio: "permitido sin duda alguna"): OpenRouter is admitted as the fourth provider of the access path, one key reaching every allowlisted model. Written no-training basis verified live 2026-09-03: prompt retention at OpenRouter is opt-in only and its prompt data has never been shared, sold or licensed; and the app enforces the exclusion itself per request — `provider.zdr: true` routes only to zero-data-retention endpoints, non-retention means no training, unclear policies default to retain-and-train, and a per-request setting can strengthen but never weaken account settings. Stricter than the gate requires (retention stays ungated) and technically enforced rather than terms-only. Full ruling and evidence: OQ-10. Spine updated: AD-9 and the deferred-topology bullet.
