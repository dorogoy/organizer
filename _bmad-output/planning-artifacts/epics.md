...
- **NFR12: Platform and stack.** Android only. Flutter 3.47.x — the latest stable patch of the line; patch bumps are routine and wanted, 3.48+ is a decision (Dart 3.13.x, the pubspec constraint `^3.13.0`). JDK 17, the major the Android toolchain verifies against (Flutter 3.47: Java 17 minimum, KGP 2.4.0) — the patch floats. Android target SDK 36, **minSdk 33** (set by `POST_NOTIFICATIONS` and by `checkRecognitionSupport()` / `triggerModelDownload()`). 64-bit ABIs only (`abiFilters` excludes 32-bit — also the 16 KB page-size condition). No `material_ui` / `cupertino_ui` dependency added.
...
**Then** the line-pinned Flutter SDK (3.47.x, Dart 3.13.x) and JDK 17 are on PATH from the devbox environment, and `devbox.json` and `devbox.lock` are committed
