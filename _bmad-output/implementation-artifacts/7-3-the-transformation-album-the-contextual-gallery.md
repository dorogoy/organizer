...
- [x] [Review][Patch] A failed refresh after a rendered mutation leaves stale album state [lib/ui/album/album_screen.dart:97] — **Severity: high.** `_read()` swallows a read failure without clearing `_entries`, so a failed refresh after `_delete()` or `_purge()` keeps the old rows and controls visible instead of returning to the quiet pending state required by the failure contract. Clear the state only for the current read generation so an older failure cannot erase a newer result.
- [x] [Review][Patch] Album rows need stable keys for their stateful `PhotoFrame` descendants [lib/ui/album/album_screen.dart:188] — **Severity: medium.** The unkeyed reversed list reuses row elements when an entry is deleted; each `PhotoFrame` can therefore retain the previous row's bytes while its asynchronous reload settles. Key each entry (or each thumbnail) by stable entry identity so a row change starts from the correct empty/loading state.
- [x] [Review][Patch] Guard the gallery's close pop during route transitions [lib/ui/album/album_screen.dart:211] — **Severity: medium.** `Cerrar` calls `Navigator.pop()` without the `ModalRoute.isCurrent` guard used by the album push and empty-pop paths. A second tap while the gallery is exiting can pop the reward route beneath it. Apply the same current-route guard to the close action.
- [x] [Review][Patch] Negative reward-arm tests omit the non-null album seam [test/ui/reward/reward_screen_test.dart:281] — **Severity: medium.** The shoot-offered and no-Before tests leave `album` null, so they pass even if the `rewardOpenAlbum` affordance leaks into either forbidden arm; the implementation's `widget.album != null` condition masks that regression. Exercise both negative arms with a real `AlbumController` and assert the affordance is absent.
- [x] [Review][Patch] The generation-guard test does not make the parked read stale [test/ui/album/album_screen_test.dart:101] — **Severity: medium.** `_GatedReadStore` reads the current log after releasing its gate, so delayed read #3 returns the same post-delete answer as read #5. The test remains green if the `_readGeneration` guard is removed. Snapshot the parked read's pre-mutation result or otherwise give each read a distinct response before releasing it.
- [x] [Review][Patch] The rapid double-tap test's second tap misses its target [test/ui/reward/reward_screen_test.dart:376] — **Severity: medium.** The targeted Flutter run emits a hit-test warning at the second tap because the text is already covered by the transition; the assertion therefore does not exercise `_openAlbum` while the reward route is non-current. Invoke the second callback through a hittable transition seam and assert that only one gallery route is present.
