...
- [x] [Review][Patch] A stale failed read clears `_completionAckWaiting` without a generation guard — a later successful commit can drop the ack; the clear is now generation-scoped (only the read whose generation still stands may clear), pinned by a queued-read test where a superseded generation's failure lands first and the current generation's commit still carries the ack [lib/ui/dispenser/dispenser_screen.dart, test/ui/dispenser/dispenser_screen_test.dart]
- [x] [Review][Patch] `HechoButton` forwards a nullable `onTap`; absent, `InkWell` is disabled instead of the 1.8 no-op — `onTap: onTap ?? () {}` restores the accepted no-op, pinned by an anatomy test asserting the `InkWell`'s handler is non-null and the tap absorbs quietly [lib/ui/dispenser/task_card.dart, test/ui/dispenser/task_card_test.dart]
- [x] [Review][Patch] The write-failure screen test never heals and taps again — `_writeInFlight` stuck true would brick Hecho with the suite green; the test now heals via a real foreground return and taps again, pinning exactly one `card_done`, the ack on the retried completion and a second haptic as the tap's witness [test/ui/dispenser/dispenser_screen_test.dart]
- [x] [Review][Patch] Write-failure ack-flag clearing is unobserved when a prior ack is visible — deleting the catch's flag/timer clears still passes; a `_FailLaterDoneStore` test completes once (ack visible, window fresh), fails the second write under it, then heals: the committed card carries nothing ack-shaped, all inside the first window's remains (mutation-verified) [lib/ui/dispenser/dispenser_screen.dart, test/ui/dispenser/dispenser_screen_test.dart]
