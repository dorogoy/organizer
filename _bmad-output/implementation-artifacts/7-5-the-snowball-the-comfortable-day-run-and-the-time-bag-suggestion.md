...
- [x] [Review][Patch] The AD-4 offset pin never discriminates stored offset from the read frame [packages/core/test/derive/comfortable_day_test.dart] — **Severity: medium.** 07:00 UTC is still 20 Aug under both +02:00 and offset 0. Rebuilt at 02:00 UTC +02:00 (wall 04:00 → 20 Aug); the same instant in the read frame is 19 Aug, so ignoring the stored offset would leave the 20th empty and the run would read 8.
- [x] [Review][Patch] Unbounded and out-of-range pocket fixtures never hold the claimed long span [packages/core/test/derive/comfortable_day_test.dart] — **Severity: medium.** `comfortableDay` already closed those sittings at 09:10, so the extra `session_ended` never judged an hour / two-hour span. Each arm is now the day's only pair (09:00→10:00 unbounded, 09:00→11:00 imported 99).
- [x] [Review][Patch] No pin that one marathon sitting makes the whole domestic day uncomfortable [packages/core/test/derive/comfortable_day_test.dart] — **Severity: medium.** Two sittings on Aug 26 — in-pocket with a done, then a 20-minute marathon — break the chain there; a fold that only required some non-marathon sitting would still have read ten.
- [x] [Review][Patch] RestOffer still has no pin for the snowball shown fact [test/dispenser/dispenser_controller_test.dart] — **Severity: medium.** The Closed pin left RestOffer unasserted. A 45-minute start at 11:20 on the crossing day now reads as RestOffer carrying `snowballProposedMinutes == 20`.
- [x] [Review][Patch] `dismissSnowball` dartdoc repeats the handoff clause [lib/dispenser/dispenser_controller.dart] — **Severity: low.** Dropped the duplicated "that day, handing the slot…" sentence.
