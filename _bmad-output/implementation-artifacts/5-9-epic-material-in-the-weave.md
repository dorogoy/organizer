...
- `packages/core/lib/log/log_entry.dart` -- add `LogKind.epicActivated` to the constant block (:100-119) and to `knownByName` (:122), and to `_isItemAct` (:650) so it converts to the existing `ItemActEntry` (:191) carrying the `(itemId, itemOrigin)` pair. No new subtype, no new `LogRecordFlaw` — the item pair's flaws already exist. Verify no other classifier or fold in the conversion (`convertLogEntryRecord:704`) needs a branch.
- `packages/core/lib/commands/scan_commands.dart` -- NEW `epicActivated({required String itemId, required Origin origin})` → one `LogEntryContent`, on `consentGranted`'s shape (:72). The slice channel's own file, shared by both entrances; the kind's single sanctioned minter.
- `packages/core/lib/weave/session.dart` -- the activation fold in `walkLog` (:208, beside `lastDealtInstantByItemId`'s branch at :344): a new insertion-ordered `LogFacts` field (:43) mapping each activated Epic's stable id to its activation instant — append order **is** activation order (AD-3). Confirm the new item-act kind disturbs no existing fold (`answeredItemIds:95`, `skippedDaysByItemId:91` key on their own kinds).
- `packages/core/lib/weave/weave.dart` -- the story's heart:
  - `CandidatePrecedence.epic` between `capture` and `catalogue` (:136) — a member, never a flag (the enum's own doc rule).
  - NEW `epicCandidates(List<PoolFact>, LogFacts)` beside `rescueCandidates` (:292): the grouping fold (`rescueCandidates`' `headsByParent` shape), the head rule, and the AD-20 arbitration expressed as the returned order; `name: fact.stepText ?? fact.originContext ?? ''`, `estimateSeconds: fact.estimateSeconds`, `zone: null`, `createdInstantUtcMicros: fact.instantUtcMicros`.
  - the spread in `_resolveDay`'s candidate list (:581-583), after `captureCandidates`.
  - the chunk pool filter (:594): `size == Size.focus || precedence == CandidatePrecedence.epic`.
  - the epic tier in `_chunkCandidateOf` (:456) — after the capture tier's return (:472), **before** the `activeZone == null` return (:474); it takes the source's first epic candidate, the source owning the Epic arbitration inside `core/weave` (AD-20's "the resolver's too").
  - the two draw exclusions (:651, :662): `precedence != rescue && precedence != epic`.
  - `cardForItem`'s naming (:891): `fact.stepText ?? fact.originContext ?? ''` — one expression for the candidate and the standing card.
- `lib/scan/scan_controller.dart` -- `_appendScanLanded` (:765): keep the first minted fact id and append the `epicActivated` row after the fact loop, inside the same queued closure; nothing else on the path moves.
- `lib/genesis/genesis_controller.dart` -- `_appendGenesisLanded` (:364): the same addition, the epoch re-checks (:372, :381) unchanged.
- Read-only evidence: `lib/store/drift_store.dart:71-83` (the order contract this story consumes), `packages/core/lib/pool/pool_fact.dart:66` (the class-not-size declaration), `packages/core/lib/commands/scan_commands.dart:183` (`scanSliceLanded`, untouched), `lib/ui/dispenser/dispenser_screen.dart:220` (`didPopNext` already re-derives), `packages/core/lib/weave/weave.dart:777` (`_guardedTierDealOf`'s ladder, untouched).
- Tests: NEW group in `packages/core/test/weave_test.dart` ("Epic material in the weave", on the rescue-chain group's shapes at :2819) covering the whole matrix; NEW fold pins in `packages/core/test/session_test.dart`; RENEGOTIATE `packages/core/test/log_test.dart:55-89` (sorted names + census 21), `test/no_lateness_proof_test.dart:462-487` and `:491-501` (`appendLogEntry` census: scan 1 → 2, genesis 1 → 2), `test/scan/scan_controller_test.dart` + `test/genesis/genesis_controller_test.dart` (one activation row per landing, none when nothing landed). VERIFY unchanged: `packages/core/test/no_lateness_proof_test.dart:2385` (the kind-name vocabulary ban — `epic_activated` passes) and `:1229` (frozen-type census — no new type), `test/weave/rotation_asset_test.dart` (no Epic facts in the asset runs, so the counts must not move).
