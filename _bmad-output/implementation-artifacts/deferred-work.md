...
## Deferred from: code review of 5-4-the-scan-cache-and-its-single-use-consent-token (2026-09-06)

- source_spec: `_bmad-output/implementation-artifacts/5-4-the-scan-cache-and-its-single-use-consent-token.md`
  summary: The app_opened sweep can unlink a LIVE scan's cache — a transient occlusion (inactive→resumed, e.g. a notification shade pulled mid-shoot) makes the session observer re-open (and sweep) while the scan surface deliberately holds the scan open (`_shooting` guard), so the in-flight shot's frame dies under the gate's read and folds into the quiet close.
  evidence: Review round 1, all three reviewers independently (blind-hunter + edge-case-hunter converge on session_controller.dart:190-194 × scan_screen.dart:123-127). Fail-closed direction, and AD-8 orders a sweep at EVERY app_opened — the conflict is between the sweep's every-open contract and the scan surface's hold-open-through-occlusion policy (5.2's). 5.6 owns departure-cancels-scan semantics; 5.5 wires the mint into the flow. Decide there: accept the fail-closed race (and possibly surface a notice instead of a quiet close, per the 5.2 retro seed), or gate the sweep on real pauses only, or have the scan surface close on occlusion. Docs in files_port/session_controller were made honest in this story; the frozen spec sentence still over-claims ("by definition a crash leftover") — renegotiation is Sergio's.

- source_spec: `_bmad-output/implementation-artifacts/5-4-the-scan-cache-and-its-single-use-consent-token.md`
  summary: Cross-scan token binding is nominal — `ScanConsent.consume()` verifies once-ness but not binding: a token minted for scan A passed with scan B's payload consumes cleanly; the payload carries no scanId to check against (deliberately, data minimization on the wire shape).
  evidence: Edge-case review 2026-09-06 (egress_dispatch.dart consume site). Unreachable from the sanctioned flow 5.5 will wire (the controller mints from the same `_scanId` it builds the request from), so it is programmer-error class like double-consumption — but unlike double-consumption it throws nothing. 5.5 decides whether construction-site identity makes it structurally impossible (e.g. a request builder closed over one scan's state) before any payload ships; adding scanId to the payload shape would weaken the "image and prompt only" wire contract and was rejected for this story.
