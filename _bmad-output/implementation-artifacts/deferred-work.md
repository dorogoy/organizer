...
## Deferred from: story 5.1 builder ruling (2026-09-05)

- source_spec: `_bmad-output/implementation-artifacts/5-1-the-on-device-face-gate-verified-before-it-is-trusted.md`
  summary: Face-gate composition (face ∨ pose ∨ object detection vs minimal face-only) deferred pending real-usage data — the interim production gate is face-only (`google_mlkit_face_detection 0.15.1`, accurate, `minFaceSize: 0.0`); the accepted interim risk is 4 FN / 12 on the final-tree corpus (partial 2/2 — p01, p02; profile 1/2 — p03; distance 1/2 — p05, reproduced in face-gate/results/diagnostics/final-tree-smoke.json), with only those four awaiting the deferred composition — the print-on-wall misses and one profile pass on the face limb alone at `0.0`, delivered by the config lever already in the interim gate.
  evidence: Story 5.1's three escalations, all ruled on by Sergio in session 2026-09-05 — run 1 face-only @ 0.05 scored 6 FN / 12 (`face-gate/results/runs/1.json`); the person-gate re-run was voided by infrastructure (pose-era seal strips broke pose at runtime, 16/16 error rows) while the lifted-build diagnostic banked face ∨ pose @ 0.0/accurate at 3 FN / 12 (`face-gate/results/diagnostics/lifted-manifest-diagnostic.json`); the final ruling weighed the cost spiral (~22 MB pose on-device, ~590 ms median dual-detector emulator latency, a second fragile dependency surface, contingent AD-11 promotion) against zero pre-5.5 upload exposure (AD-8's compile-time `ScanConsent` precondition) and better data from 5.2 usage. The OD rung was never measured; probe v1 + v2 are preserved at `face-gate/probe/` for the reopen. Must be settled before story 5.5 ships the first scan payload (AD-8 ordering).
- source_spec: `5-1-the-on-device-face-gate-verified-before-it-is-trusted.md`
  summary: No standing check guards the fragile-dependency exact pins (pubspec.yaml pin + pubspec.lock resolution vs the spine Stack table)
  evidence: Verification-gap review 2026-09-05 — relaxing `google_mlkit_face_detection` to `^0.15.1` and resolving 0.15.2 leaves make gate/check/CI green (seal 2 sees only Maven coordinates, which need not move), shipping a detector version the corpus measurement never certified; the story's Never clause (no permanent tool/ checks) forbade adding it inside 5.1
- source_spec: `5-1-the-on-device-face-gate-verified-before-it-is-trusted.md`
...
- source_spec: `_bmad-output/implementation-artifacts/5-9-epic-material-in-the-weave.md`
  summary: Reconcile the epoch/cancellation discipline between the scan and typed-genesis landing methods — `ScanController._appendScanLanded` never re-checks `_epoch` (in its fact loop or before its `epic_activated` append), while `GenesisController._appendGenesisLanded` re-checks it at both points, and callers mirror the same asymmetry (genesis re-checks `_epoch` after the landing call; scan does not).
  evidence: Story 5.9 code review (blind-hunter, 2026-09-08); the asymmetry predates this story — both landing methods inherited their own pre-existing epoch convention when 5.9 added the `epic_activated` append to each — but the two controllers implementing the same story's landing contract now diverge visibly on a documented safety property (`lib/scan/scan_controller.dart:769-825` vs `lib/genesis/genesis_controller.dart:369-436`).
- source_spec: `_bmad-output/implementation-artifacts/5-12-curation-s-other-two-homes-the-e1-surface-and-the-one-time-s.md`
  ...
  resolved: Story 6-6 (`_bmad-output/implementation-artifacts/6-6-the-blind-six-month-follow-up.md`, review patch) — the `_stripAct` extraction routes every seam-routed dismissal (`_onDismissCheckIn`, `_onDismissReport`, `_onDismissQuarantineFollowUp`) through one empty-frame failure arm, and `test/ui/dispenser/ambient_strip_test.dart` now drives a ✕ over a `_FailReadWhileArmedStore` with reads failing: the quiet empty frame (no ErrorWidget), zero appended rows, and the healed foreground re-read showing the post-dismissal state — the arm is covered for the paths that reach it.
- source_spec: `_bmad-output/implementation-artifacts/5-13-the-gentle-seasonal-suggestion.md`
  ...
  resolved: Story 6-6 (`_bmad-output/implementation-artifacts/6-6-the-blind-six-month-follow-up.md`) extracts `_stripAct` in `dispenser_screen.dart` — one seam with the two failure policies (recovery-read vs empty-frame) — and routes `_onSetEnergy`, `_onDismissCheckIn`, `_onAnswerReport`, `_onDismissReport`, both suggestion paths and the new `_onDismissQuarantineFollowUp` through it; the five families' existing suites stay green.
- source_spec: `_bmad-output/implementation-artifacts/5-13-the-gentle-seasonal-suggestion.md`
...
- source_spec: `_bmad-output/implementation-artifacts/6-4-three-destinations-of-equal-weight.md`
  summary: A failed destination-act write leaves the 3-Destination Flow's rows permanently inert until back + full protocol re-entry, while `Hecho`'s failed write allows in-place retry.
  evidence: Story 6-4 review (blind hunter): the flow's `_handedOff` one-shot never resets on `landed == false`, so after a quiet write failure three visually live rows are dead with no busy/failed distinction — the frozen matrix names re-entry as the retry, a deliberate anti-overwhelm choice, but it diverges from `_onDone`'s in-place retry precedent; a one-line reset on failure would match the house shape if the human prefers it.
