...
- source_spec: `_bmad-output/implementation-artifacts/5-12-curation-s-other-two-homes-the-e1-surface-and-the-one-time-s.md`
  summary: The 2.5-era check-in dismissal handler's read-failure arm (`_onDismissCheckIn` in dispenser_screen.dart) is untested — same untested shape story 5.12 avoided adding for its own two new handlers.
  evidence: Story 5.12's verification-gap review confirmed no test drives any strip dismissal over a throwing `readLogEntries`; the new accept/dismiss arms got a failing-read test in 5.12, the check-in's pre-existing arm never had one.
