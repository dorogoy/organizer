...
- [x] [Review][Patch] The core mint census permits a second `card_done` or `card_dealt` writer in `log_entry.dart`; the definition, registry and classifier references are now exact-count pins, so the one minter remains `session_commands.dart` [packages/core/test/no_lateness_proof_test.dart]
- [x] [Review][Patch] The exhaustive shape census missed generic record typedefs and extension types; both declaration forms are now scanned and self-pinned [packages/core/test/no_lateness_proof_test.dart]
- [x] [Review][Patch] The source scanners stripped `//` before masking strings, so a same-line URL/string could hide later tokens; both local scanner copies now lex strings before nested comments and pin that behavior [packages/core/test/no_lateness_proof_test.dart, test/no_lateness_proof_test.dart]
- [x] [Review][Patch] The deferred-chunk proof did not snapshot the log around `composeDay`; it now asserts that the day-N to day-N+1 gap adds zero rows [packages/core/test/weave_test.dart]
- [x] [Review][Patch] The absence furniture census omitted semantic hints and values; all semantic text properties are now compared with the control [test/ui/dispenser/dispenser_screen_test.dart]
