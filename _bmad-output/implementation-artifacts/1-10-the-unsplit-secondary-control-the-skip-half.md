...
- [x] [Review][Patch] The write guard releases before the refreshed view commits [lib/ui/dispenser/dispenser_screen.dart:199] — the shared guard now persists through the refresh's removal frame, and a queued-read screen test pins that a stale `Hecho` callback fires neither haptic nor acknowledgement.
- [x] [Review][Defer] Lifecycle writes can split a skip batch [lib/dispenser/dispenser_controller.dart:162] — `SessionController.handleSessionEnd` has a separate write queue and can append `session_ended` after `card_skipped` but before the bundled `card_dealt`, leaving that deal outside an open session. This extends the existing 1.9 serialization deferral and needs a shared coordinator or an explicit display invariant outside this story's no-session-controller-change boundary.
