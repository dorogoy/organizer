...
// the v2 instrument at face-gate/probe/ — sha256-recorded in
// face-gate/results/report.md; restores to test/face_gate_probe_test.dart
// (see the report's restore recipe; v1 and v2 collide on filenames).
import 'dart:convert';
import 'dart:io';
...
import 'face_gate_probe_v2_test.dart' as probe;
...
void main() {
  ...
  test('a missing pose-detector-config block refuses the run', () {
    expect(
      () => probe.parsePoseDetectorOptions(
        barWithBlockSwapped(
          probe.poseDetectorConfigBegin,
          probe.poseDetectorConfigEnd,
          '',
        ),
      ),
      throwsStateError,
  ...
  test('an unknown pose model or mode refuses the run', () {
    final badModel = confirmedBar.replaceFirst(
      'model: accurate',
      'model: premium',
    );
    expect(() => probe.parsePoseDetectorOptions(badModel), throwsStateError);
    final badMode = confirmedBar.replaceFirst('mode: single', 'mode: batch');
    expect(() => probe.parsePoseDetectorOptions(badMode), throwsStateError);
  });

  test('the pinned pose configuration parses into the exact options', () {
    final options = probe.parsePoseDetectorOptions(confirmedBar);
    expect(options.model, PoseDetectionModel.accurate);
    expect(options.mode, PoseDetectionMode.single);
  });

  test('the amended gate rule refuses on either detector: verdicts for '
      'every faces/poses combination', () {
    // person: refused by either detector is a gate-pass.
    expect(probe.verdictFor(isPerson: true, faces: 1, poses: 0), 'gate-pass');
    expect(probe.verdictFor(isPerson: true, faces: 0, poses: 1), 'gate-pass');
    expect(probe.verdictFor(isPerson: true, faces: 2, poses: 3), 'gate-pass');
    // person missed by both is the false negative.
    expect(
      probe.verdictFor(isPerson: true, faces: 0, poses: 0),
      'false-negative',
    );
    // no-person: refused by either detector is the accepted FP.
    expect(
      probe.verdictFor(isPerson: false, faces: 1, poses: 0),
      'false-positive',
    );
    expect(
      probe.verdictFor(isPerson: false, faces: 0, poses: 1),
      'false-positive',
    );
    expect(probe.verdictFor(isPerson: false, faces: 0, poses: 0), 'clean-pass');
  });
