...
//
// Story 5.1's throwaway measurement instrument — deleted once the
// evidence pack captures its results, then preserved verbatim at
// face-gate/probe/face_gate_probe_test.dart. It measures what story 5.2
// will ship: the spine-pinned plugin (google_mlkit_face_detection
// 0.15.1) running on-device with the detector configuration frozen in
// face-gate/BAR.md — the gate is trusted from this measurement, never
// from the plugin's README.
//
// Contract (the story's spec): reads BAR.md and the corpus from the
// app's external-files dir (adb-pushed there; no permissions needed),
// refuses to score without the bar's dated builder confirmation,
// validates the corpus floor, scores every photo through the pinned
// FaceDetectorOptions, writes per-image rows plus the FN/FP summary to
...
/// Scores one photo: a detection error is retried once, then the row is
/// declared `error` — excluded from the FN/FP denominators, reported,
...
Future<Map<String, Object?>> scorePhoto(
  ...
  var faces = 0;
  var attempts = 0;
  while (attempts < maxAttemptsPerPhoto && !scored) {
    attempts++;
    try {
  ...
  final verdict = !scored
      ? 'error'
      : photo.isPerson
      ? (faces > 0 ? 'gate-pass' : 'false-negative')
      : (faces > 0 ? 'false-positive' : 'clean-pass');
  return <String, Object?>{
    ...photo.toJson(),
    'faces': scored ? faces : null,
    'verdict': verdict,
    'attempts': attempts,
...
void main() {
  ...
  testWidgets('face-gate probe — the pinned detector over the corpus', (
    ...
    final photosDir = Directory('${baseDir.path}/corpus/$photosDirName');
    print(
      'bar confirmed $confirmedOn — ${photos.length} photos, detector '
      '${options.toJson()}',
    );
    ...
    try {
      for (final photo in photos) {
        final row = await scorePhoto(detector, photosDir, photo);
        rows.add(row);
        print(
    ...
    expect(
      ...
      reason:
          ...
          'passed the detector with zero faces — escalate with the two '
          'recorded remedy options (AD-11 platform-channel promotion; '
          'tighter detection configuration), decided against the '
          'measurement, never absorbed as tuning. Results are in the '
          'console copy above.',
    );
