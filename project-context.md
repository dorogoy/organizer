...
- **Build JVM: a current JDK LTS from devbox** (nixpkgs `jdk21` today; 25 once the template's Gradle is 9.1+ — Gradle needed 9.1 for Java 25). 17 is the toolchain's *minimum*, not a pin — pinning to an aging minimum buys nothing; a non-LTS is never used. The **emitted bytecode** stays at Java 17 / Kotlin `jvmTarget` 17 — that build-JVM/bytecode split is the ruling. Beware in devbox: the plain nixpkgs `jdk` package is JDK 8.
